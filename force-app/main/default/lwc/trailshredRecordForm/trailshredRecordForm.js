        ...
        if (this.trailshredFieldValueSettings && this.trailshredFieldValueSettings.length > 0) {
            ...
            if (oldRecord && newRecord) {
                ...
                this.trailshredFieldValueSettings.forEach(setting => {
                    ...
                    // then convert value to string so we have common datatype to compare with
                    ...
                    if (
                        fieldValueChanged &&
                        (newFieldValueMatchesTargetValue || newFieldDisplayValueMatchesTargetValue)
                    ) {
                        let cacheBuster = new Date().getTime();
                        audios.push(
                            new Audio("/resource/" + cacheBuster + "/" + setting.Audio_Static_Resource_Path__c)
                        );
                    }
                });
            }
